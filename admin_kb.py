...
async def approve_ad(callback: types.CallbackQuery):
    ...
    # Редактируем подпись к видео, чтобы показать, что реклама подтверждена
    await callback.message.edit_caption(caption="Реклама подтверждена.")
    await callback.answer()
    
    # Здесь вызываешь функцию или логику для отправки на монетизацию
...
async def reject_ad(callback: types.CallbackQuery):
    ...
    # Логика, если нужно что-то сделать при отклонении
    await handle_reject(video_id)


# Пример заглушек для логики после модерации
async def send_to_monetization(video_id):
    # Тут твоя логика для обработки видео, отправки на монетизацию и т.д.
    pass

async def handle_reject(video_id):
    # Логика при отклонении, если нужна
    pass
...
async def send_broadcast(message: types.Message, state: FSMContext):
    if callbacks.from_user.id not in ADMIN_IDS:
        return
    text = message.text
    success, failed = 0, 0
    async with aiosqlite.connect(DB_PATH) as db:
